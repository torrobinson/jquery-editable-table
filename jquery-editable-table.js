...
let backgroundColorFromValue = function (value, cell) {
    $(cell).removeClass('good').removeClass('medium').removeClass('bad');
    if (value > 0 && value <= 3) {
        $(cell).addClass('good');
    }
    ...
    columns: [
        ...
        },
        {
            ...
            convertOut: (outgoingVal) => {
                ...
                return parseInt(outgoingVal);
            },
            isValid: (val, data) => {
                // Ensure typed values are valid numbers
                val = parseInt(val);
                return !isNaN(val) && Number.isInteger(val);
            },
            afterAdd: (addedVal, td) => {
                // After a new row is added or data is initially set
                backgroundColorFromValue(addedVal, td);
            },
            afterChange: (newValue, td) => {
                backgroundColorFromValue(newValue, td);
            }
        }
    ]
};
